...
class Pantry extends CI_Controller {
	...
	public function getAll()
	{
		...
		try{
			...
			$res = $this->Model_Api->getAllTrsPantry($nik);
	...
	public function getPlace()
	{
		...
		try{
			...
			$res = $this->Model_Api->select_all_data('pantry', array('is_deleted' => 0),
				array('id pantry_id', 'name', 'pic', 'detail','location','days','opening_hours_start','opening_hours_end'), 'result' );
			$response = response("success", $res, "Success get data to pantry place2 ");
			echo $response ;
	...
	public function postSubmitOrder()
	{
		try{
			...
			if($booking_id == "" || $pantry_id == ""){
				$response = response("fail",array(), "Pantry or schedule is empty");
				echo $response ;
				die();
			}
			$nik_order = $post['nik'];
			...
			foreach ($post['pantryOrder'] as $key => $value) {
				...
				foreach ($value['detailorder'] as $k => $v) {
					...
					foreach ($ddd as $ki => $vi) {
						if($vi['onchange'] == 1){
							array_push($data_variant_detail, $post['pantryOrder'][$key]['detailorder'][$k]['variant_detail'][$ki]);
						}
					}
					$post['pantryOrder'][$key]['detailorder'][$k]['variant_detail'] = $data_variant_detail;
				}
		...
		try{
			...
			if($res['error'] == null){
				...
				foreach ($col as $key => $value) {
					if($value['detailorder'] == "" || $value['detailorder'] == "[]"){
						$col[$key]['detailorder'] = [];
					}else{
						$D = $value['detailorder'];
						$col[$key]['detailorder'] = json_decode($D, TRUE);
					}
				}
				$response = response("success", $col, "Success get detail to pantry ");
				echo $response;
