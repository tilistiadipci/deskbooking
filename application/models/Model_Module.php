...
class Model_Module extends CI_Model {
	...
	// public()
	// public()
	public function get_module_int_alarm() {
		$tbl = $this->get_table();
		$this->db->select('*')
		->from($tbl)
		->where(array(
			"module_text" => "module_int_alarm"
		));
		$ret = $this->db->get()->row_array();	
		return $ret;
	}
	public function get_module_int_365() {
		$tbl = $this->get_table();
		$this->db->select('*')
		->from($tbl)
		->where(array(
			"module_text" => "module_int_365"
		));
		$ret = $this->db->get()->row_array();	
		return $ret;
	}
	public function get_module_int_google() {
		$tbl = $this->get_table();
		$this->db->select('*')
		->from($tbl)
		->where(array(
			"module_text" => "module_int_google"
		));
		$ret = $this->db->get()->row_array();	
		return $ret;
	}
	
	public function get_module_floor() {
		$tbl = $this->get_table();
		$this->db->select('*')
		->from($tbl)
		->where(array(
			"module_text" => "module_floor"
		));
		$ret = $this->db->get()->row_array();	
		return $ret;
	}
	public function get_module_automation() {
		$tbl = $this->get_table();
		$this->db->select('*')
		->from($tbl)
		->where(array(
			"module_text" => "module_automation"
		));
		$ret = $this->db->get()->row_array();	
		return $ret;
	}

	public function get_module_pantry() {
		$tbl = $this->get_table();
		$this->db->select('*')
		->from($tbl)
		->where(array(
			"module_text" => "module_pantry"
		));
		$ret = $this->db->get()->row_array();	
		return $ret;
	}
	public function get_module_vip() {
		$tbl = $this->get_table();
		$this->db->select('*')
		->from($tbl)
		->where(array(
			"module_text" => "module_user_vip"
		));
		$ret = $this->db->get()->row_array();	
		return $ret;
	}

	public function get_module_room() {
		$tbl = $this->get_table();
		$this->db->select('*')
		->from($tbl)
		->where(array(
			"module_text" => "module_room"
		));
		$ret = $this->db->get()->row_array();	
		return $ret;
	}
	public function get_module_room_adv() {
		$tbl = $this->get_table();
		$this->db->select('*')
		->from($tbl)
		->where(array(
			"module_text" => "module_room_advance"
		));
		$ret = $this->db->get()->row_array();	
		return $ret;
	}
	public function get_module_display() {
		$tbl = $this->get_table();
		$this->db->select('*')
		->from($tbl)
		->where(array(
			"module_text" => "module_display"
		));
		$ret = $this->db->get()->row_array();	
		return $ret;
	}
	public function get_module_alocation() {
		$tbl = $this->get_table();
		$this->db->select('*')
		->from($tbl)
		->where(array(
			"module_text" => "module_alocation"
		));
		$ret = $this->db->get()->row_array();	
		return $ret;
	}
	public function get_module_access_door() {
		$tbl = $this->get_table();
		$this->db->select('*')
		->from($tbl)
		->where(array(
			"module_text" => "module_access_door"
		));
		$ret = $this->db->get()->row_array();	
		return $ret;
	}
	public function get_module_booking() {
		$tbl = $this->get_table();
		$this->db->select('*')
		->from($tbl)
		->where(array(
			"module_text" => "module_booking"
		));
		$ret = $this->db->get()->row_array();	
		return $ret;
	}
	public function get_module_email() {
		$tbl = $this->get_table();
		$this->db->select('*')
		->from($tbl)
		->where(array(
			"module_text" => "module_email"
		));
		$ret = $this->db->get()->row_array();	
		return $ret;
	}
	public function get_module_invoice() {
		$tbl = $this->get_table();
		$this->db->select('*')
		->from($tbl)
		->where(array(
			"module_text" => "module_invoice"
		));
		$ret = $this->db->get()->row_array();	
		return $ret;
	}
	public function get_module_price() {
		$tbl = $this->get_table();
		$this->db->select('*')
		->from($tbl)
		->where(array(
			"module_text" => "module_price"
		));
		$ret = $this->db->get()->row_array();	
		return $ret;
	}
	public function get_module_beacon() {
		$tbl = $this->get_table();
		$this->db->select('*')
		->from($tbl)
		->where(array(
			"module_text" => "module_beacon"
		));
		$ret = $this->db->get()->row_array();	
		return $ret;
	}
	public function get_module_kiosk() {
		$tbl = $this->get_table();
		$this->db->select('*')
		->from($tbl)
		->where(array(
			"module_text" => "module_kiosk"
		));
		$ret = $this->db->get()->row_array();	
		return $ret;
	}
	public function get_module_loker() {
		$tbl = $this->get_table();
		$this->db->select('*')
		->from($tbl)
		->where(array(
			"module_text" => "module_loker"
		));
		$ret = $this->db->get()->row_array();	
		return $ret;
	}

	public function get_module_core() {
		$tbl = $this->get_table();
		$this->db->select('*')
		->from($tbl)
		->where(array(
			"module_text" => "module_core"
		));
		$ret = $this->db->get()->row_array();	
		return $ret;
	}
	
	public function get_module_desk() {
		$tbl = $this->get_table();
		$this->db->select('*')
		->from($tbl)
		->where(array(
			"module_text" => "module_desk"
		));
		$ret = $this->db->get()->row_array();	
		return $ret;
	}
