        ...
        if(!empty($headers) AND is_array($headers)) {
            ...
            foreach ($headers as $header_name => $header_value) {
                if (strtolower(trim($header_name)) == strtolower(trim($this->token_header))){
                    ...
                    if(strtolower(trim($this->token_type)) == "vanila"){
                        return ['status' => TRUE, 'token' => $header_value];
                    }else if(strtolower(trim($this->token_type)) == "bearer"){
                        $bearer_sp = explode(" ", $header_value);
                        if(strtolower(trim($this->token_type)) == strtolower(trim($bearer_sp[0]))){
                            if(isset($bearer_sp[1])){
                                $value_token = $bearer_sp[1];
                                // echo $value_token;
                                return ['status' => TRUE, 'token' => $value_token];
