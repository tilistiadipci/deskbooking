...
<body style="margin: 0px !important;"  >
	...
	<div id="id_template_area" style=" background-color:#00363a; padding: 5px;">
		...
		<div style="height: 740px; background-color:white;width: 100%;">
			...
			<div >
				<img id="id_image_map" src="<?= base_url()?>assets/file/room/1942081.png" style="height: 600px;width: 1066px; ">
			</div>
		</div>
	...
	<script type="text/javascript">
		...
		function initMap(){
			...
			var point = getParameterByName('point')
                $.ajax({
                    url : "/get/route-map-wayfinding/"+x ,
                    type : "get",
                    data : "",
                    dataType : "json",
                    beforeSend: function(){
                    ...
                    success: function(resp){
                        ...
                        if (resp.status == "success") {
                       		var map = data[0]
                       		gTemplate.height = map.display_height;
                       		gTemplate.width = map.display_width;
                       		gTemplate.image = map.map;
                       		$('#id_template_area').css({
                       			'height': gTemplate.height+"px",
                       			'width': gTemplate.width+"px",
                       		})
                       		var urlmap = "/media/wayfinding/"+gTemplate.image;
                       		$('#id_image_map').attr('src', urlmap)
                       		if(point != "" || point != null){
                       			onBootAddPoint(point);
                       			$('#id_booth_input').val(point);
                       		}
                       		// console.log(map);
                        }
                    ...
                    error: function(){
                        var text = "Error Connection, Please check connection";
                        showNotification("alert-danger", text, "top", "center", "animated zoomInUp", "animated zoomOutUp")
                    }
                })
            }
	</script>
