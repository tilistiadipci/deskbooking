        ...
        var search_startDate = moment();
        var search_endDate = moment();
        var levelid = $('#id_level').val()
        var userlogin = $('#id_user').val()
        ...
        function getCrtBookingHTML(){
            var bs = $('#id_baseurl').val();
            $.ajax({
                url : bs+"desktrs/get/booking-create/page1",
                type : "GET",
                beforeSend: function(){
        ...
        function exportPDF(){
            ...
            var search = {
                ...
                startDate: search_startDate.format("YYYY-MM-DD"),
                endDate: search_endDate.format("YYYY-MM-DD"),
            };
            var url = bs+"desktrs/export/pdf?";
            url += `status=${search_status}`;
            url += `&startDate=${search_startDate.format("YYYY-MM-DD")}`;
            url += `&endDate=${search_endDate.format("YYYY-MM-DD")}`;
        ...
        function exportExcell(){
            ...
            var url = bs+"desktrs/export/excell?";
            url += `status=${search_status}`;
            url += `&startDate=${search_startDate.format("YYYY-MM-DD")}`;
            url += `&endDate=${search_endDate.format("YYYY-MM-DD")}`;
                ...
                success:function(data){
                    if(data.status == "success"){
                        ...
                        $.each(data.collection, function(index, item){
                            ...
                            var datebooking = moment(item.date).format("MMM DD, YYYY");

                            if(moment().unix() > moment(item.end).unix()){
                                statusHtml = "<span class='status-pill pill-expired'>Expired</span>";
                            }else if(item.end_early_meeting == 1){
                                bookRun = true;
                                statusHtml = "<span class='status-pill pill-expired'>Ended Early</span>";
                            }else if(
                                    (moment().unix() <= moment(item.end).unix() &&  
                                    moment().unix() >= moment(item.start).unix()) || item.status == "active" 
                                ){
                                bookRun = true;
                                statusHtml = "<span class='status-pill pill-active'>Active</span>";
                            }else if(
                                moment().unix() <= moment(item.date+" "+item.start).unix() 
                            ){
                                statusHtml = "<span class='status-pill pill-soon'>Soon</span>";
                            }
                            if(item.status == "soon"){
                                if(item.is_rescheduled == 1){
                                    statusHtml = "<span class='status-pill pill-soon'>Rescheduled</span>";
                                }
                            }
                            if(item.status == "cancel"){
                                statusHtml = "<span class='status-pill pill-canceled'>Canceled</span>";
                            }
                            ...
                            // Process PIC Initial
                            var picName = item.pic ? item.pic : "Unknown";
                            var picInitials = picName.substring(0, 2).toUpperCase();
                            var avatarColors = ['c-blue', 'c-red', 'c-green', 'c-orange', 'c-purple'];
                            var colorClass = avatarColors[numm % avatarColors.length];

                            html += '<tr data-id="'+item.booking_id+'">';
                            html += '<td>'+numm+'</td>';
                            html += '<td><a href="#" class="id-link">'+item.booking_id+'</a></td>';
                            html += '<td data-field="name" class="title-text">'+item.title+'</td>';
                            
                            html += '<td>';
                            html += '  <div class="icon-text-row">';
                            html += '    <i class="material-icons">domain</i>';
                            html += '    <div class="icon-text-content">';
                            html += '      <span>' + item.room_name + '</span>';
                            html += '      <span class="content-sub">' + item.desk_name + '</span>';
                            html += '    </div>';
                            html += '  </div>';
                            html += '</td>';
                            ...
                            html += '    <div class="icon-text-content">';
                            html += '      <span>' + datebooking + '</span>';
                            html += '      <span class="content-sub" style="color:#757575;">' + $start + ' - ' + $end + '</span>';
                            html += '    </div>';
                            html += '  </div>';
                            html += '</td>';
                            ...
                            html += '    <div class="avatar-circle ' + colorClass + '">' + picInitials + '</div>';
                            html += '    <div class="icon-text-content">';
                            html += '      <a onclick="clickPIC($(this))" data-id="'+item.booking_id+'" style="cursor:pointer;font-weight:600;color:#333;">'+picName+'</a>';
                            html += '      <span style="font-size:12px;color:#757575;">Divisi IT</span>';
                            html += '    </div>';
                            html += '  </div>';
                            html += '</td>';
                            ...
                            html += '    <button class="action-dropdown-btn" type="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="true">';
                                ...
                                if(item.is_canceled == 0 ){
                                    if(bookRun){
                                        if(item.end_early_meeting == 0){
                                             html += '<li><a href="javascript:void(0);" onclick="extendMeeting($(this))" data-id="'+item.id+'" data-booking_id="'+item.booking_id+'" data-name="'+item.title+'"><i class="material-icons">update</i> Extend Meeting</a></li>';
                                             html += '<li><a href="javascript:void(0);" onclick="endMeeting($(this))" data-id="'+item.id+'" data-booking_id="'+item.booking_id+'" data-name="'+item.title+'"><i class="material-icons">stop</i> End Meeting</a></li>';
                                        }
                                    }else{
                                        html += '<li><a href="javascript:void(0);" onclick="rescheduleMeeting($(this))" data-booking_id="'+item.booking_id+'" data-date="'+item.date+'" data-start="'+item.start+'" data-end="'+item.end+'" data-name="'+item.title+'" data-room_name="'+item.room_name+'" data-room_id="'+item.room_id+'"><i class="material-icons">event_note</i> Reschedule</a></li>';
                                        html += '<li role="separator" class="divider"></li>';
                                        html += '<li><a href="javascript:void(0);" onclick="cancelMeeting($(this))" data-id="'+item.id+'" data-booking_id="'+item.booking_id+'" data-name="'+item.title+'" style="color:#C62828;"><i class="material-icons">cancel</i> Cancel</a></li>';
                                    }
                                }
                            }
        ...
        function clickPIC(t){
            ...
            var html = '<div class="row clearfix">\
             ...
             $.ajax({
                url : bs+"desktrs/get/data/pic/"+id,
                type : "GET",
                dataType: "json",
                beforeSend: function(){
                },
        ...
        async function getCrtBookingHTML_page2(){
            ...
            $.ajax({
                ...
                success:function(data){
                    ...
                    var imageurl = image == null || image == "" ? "https://via.placeholder.com/960x540?text=No+Floor+Plan+Available" : bs+'file/room/'+room.image;
                    
                    var $img = $('#id_frm_crt_image');
                    $img.off('load').on('load', function() {
                        var isPortrait = this.naturalHeight > this.naturalWidth;
                        var w = isPortrait ? 540 : 960;
                        var h = isPortrait ? 640 : 540;
                        $(this).css({width: w + 'px', height: h + 'px'});
                        $('#map_layer').css({width: (w + 40) + 'px', height: (h + 40) + 'px'});
                    });
        ...
        window.renderMapDesks = function() {
            var overlayContainer = $('#desk_overlays');
            overlayContainer.empty();
            
            currentMapZoom = 1;
            $('#map_layer').css('transform', 'scale(' + currentMapZoom + ')');
            
            if (!gZoneDesk) return;

            for(var o in gZoneDesk) {
                var zone = gZoneDesk[o];
                if (!zone || !zone.desk) continue;
                for(var d in zone.desk) {
                    var desk = zone.desk[d];
                    var deskStatus = "available"; 
                    
                    var x = (desk.pointer_desk_x ? (desk.pointer_desk_x - 40) : (Math.random() * 800 + 50));
                    var y = (desk.pointer_desk_y ? (desk.pointer_desk_y - 10) : (Math.random() * 400 + 50));
                    var w = 30;
                    var h = 30;
                    
                    var isEnabled = desk.is_enable == 1 ;
                    var deskStatus = "available"; 
                    if (!isEnabled) {
                        deskStatus = "disabled";
                    } else {
                        if (desk.today_bookings && desk.today_bookings.length > 0) {
                            var now = moment();
                            var selectedDate = gBookingCrt.date === "today" ? moment().format("YYYY-MM-DD") : gBookingCrt.date;
                            var isOccupied = false;
                            var isReserved = false;
                            for (var b = 0; b < desk.today_bookings.length; b++) {
                                var s = moment(selectedDate + ' ' + desk.today_bookings[b].start, 'YYYY-MM-DD HH:mm:ss');
                                var e = moment(selectedDate + ' ' + desk.today_bookings[b].end, 'YYYY-MM-DD HH:mm:ss');
                                
                                if (now.isBetween(s, e) || now.isSame(s)) {
                                    isOccupied = true;
                                } else if (s.isAfter(now)) {
                                    isReserved = true;
                                }
                            }
                            if (isOccupied) deskStatus = "occupied";
                            else if (isReserved) deskStatus = "reserved";
                        }
                    }
                    
                    var pointerCss = "";
                    if (deskStatus == "disabled") pointerCss = "background-color: #999; cursor: not-allowed; border-color: #666;";
                    else if (deskStatus == "occupied") pointerCss = "background-color: #F44336; border-color: #B71C1C; color: white;";
                    else if (deskStatus == "reserved") pointerCss = "background-color: #FF9800; border-color: #E65100; color: white;";
                    
                    var clickEvent = (deskStatus == "disabled") ? "" : 'onclick="selectDeskFromMap(\'' + zone.zone_id + '\', \'' + desk.desk_id + '\', \'' + desk.block_number + '\')"';
                    
                    var html = '<div class="desk-point ' + deskStatus + '" ' +
                                     'id="map_desk_' + desk.desk_id + '" ' +
                                     'data-zone="' + zone.zone_id + '" ' +
                                     'data-desk="' + desk.desk_id + '" ' +
                                     'data-block="' + desk.block_number + '" ' +
                                     'style="left:' + x + 'px; top:' + y + 'px; width:' + w + 'px; height:' + h + 'px; ' + pointerCss + '" ' +
                                     clickEvent + '>' +
                                     desk.block_number +
                                 '</div>';
        ...
        window.selectDeskFromMap = function(zone_id, desk_id, block_number) {
            $('#id_frm_crt_position').val(zone_id);
            $('#id_frm_crt_position').selectpicker('refresh');
            changePosition($('#id_frm_crt_position')); 
            
            setTimeout(function() {
                $('#id_frm_crt_desk_number').val(desk_id);
                $('#id_frm_crt_desk_number').selectpicker('refresh');
                changeDeskNumber($('#id_frm_crt_desk_number')); 
                
                // Load timeline inline
                loadTimelineInline();
            }, 300);
        };

        window.updateSelectedDeskUI = function(desk_id, block_number, zone_id) {
            $('#sel_desk_number').text(block_number);
            var zoneName = "";
            for(var o in gZoneDesk) {
                if(gZoneDesk[o].zone_id == zone_id) zoneName = gZoneDesk[o].name;
            }
            $('#sel_desk_location').html(zoneName);
        };
        ...
        function changeDeskNumber(t){
            ...
            // Sync with map
            $('.desk-point').removeClass('selected');
            if (v != "") {
                $('#map_desk_' + v).addClass('selected');
                var blk = $('#id_frm_crt_desk_number option:selected').text();
                var zn = $('#id_frm_crt_position').val();
                updateSelectedDeskUI(v, blk, zn);
            } else {
                $('#sel_desk_number').text('-');
                $('#sel_desk_location').text('-');
            }
            ...
            var calendarTimeline = null;
        window.loadTimelineInline = function() {
            var desk_id = $('#id_frm_crt_desk_number').val();
            if (!desk_id) {
                $('#time_selection_row').hide();
                return;
            }
            
            // Show the time selection row container
            ...
            $.ajax({
                ...
                success:function(data){
                   ...
                   if(data.status == "success"){
                        ...
                        setTimeout(function() {
                            initFullCalendarTimeline(d, gTimerange);
                        }, 200);
                   }
                }
            });
        ...
        function initFullCalendarTimeline(dateStr, timeRange) {
            var calendarEl = document.getElementById('calendar-timeline');
            if (calendarTimeline) {
                calendarTimeline.destroy();
            }
            // Force clear the container to prevent duplicate renders
            calendarEl.innerHTML = '';
            
            // Build events from timeRange where book == 1
            var events = [];
            var currentEvent = null;
            
            for (var i = 0; i < timeRange.length; i++) {
                var slot = timeRange[i];
                if (slot.book == 1) {
                    if (!currentEvent) {
                        currentEvent = {
                            resourceId: 'desk',
                            title: 'BOOKED',
                            start: dateStr + 'T' + slot.time_array,
                            end: null,
                            editable: false,
                            backgroundColor: '#757575',
                            borderColor: '#424242',
                            textColor: '#ffffff',
                            classNames: ['booked-time-slot']
                        };
                    }
                } else {
                    if (currentEvent) {
                        currentEvent.end = dateStr + 'T' + slot.time_array;
            ...
            if (currentEvent) {
                currentEvent.end = dateStr + 'T23:59:00';
                events.push(currentEvent);
            }
            
            var minTime = gBookingCrt.work_start || '00:00:00';
            if (dateStr == moment().format('YYYY-MM-DD')) {
                var nowTimeStr = moment().startOf('hour').format('HH:mm:ss');
                if (nowTimeStr > minTime) {
                    minTime = nowTimeStr;
                }
            }
            var maxTime = gBookingCrt.work_end || '24:00:00';

            calendarTimeline = new FullCalendar.Calendar(calendarEl, {
                ...
                resources: [
                    { id: 'desk', title: 'Desk ' + $('#id_frm_crt_desk_number option:selected').text() }
                ],
                events: events,
                selectable: true,
                selectOverlap: false,
                eventOverlap: false, // Prevent dragging over existing bookings
                select: function(info) {
                    var mStart = moment(info.start);
                    var mEnd = moment(info.end);
                    
                    // Enforce minimum 15 minutes selection on single click
                    if (mEnd.diff(mStart, 'minutes') < 15) {
                        mEnd = mStart.clone().add(15, 'minutes');
                    }
                    
                    var startStr = mStart.format('HH:mm');
                    var endStr = mEnd.format('HH:mm');
                    
                    $('#id_frm_crt_time_start').html(startStr);
                    $('#id_frm_crt_time_end').html(endStr);
                    $('#btn_start_trig').attr('data-time', startStr);
                    $('#btn_end_trig').attr('data-time', endStr);
                    
                    // Remove previous selection event if exists
                    var oldSelection = calendarTimeline.getEventById('current-selection');
                    if (oldSelection) oldSelection.remove();
                    
                    // Add new selection event visually
                    calendarTimeline.addEvent({
                        id: 'current-selection',
                        resourceId: 'desk',
                        title: '',
                        start: mStart.format(),
                        end: mEnd.format(),
                        classNames: ['custom-selection-event']
                    });
                },
                eventResize: function(info) {
                    if (info.event.id === 'current-selection') {
                        var startStr = moment(info.event.start).format('HH:mm');
                        var endStr = moment(info.event.end).format('HH:mm');
                        $('#id_frm_crt_time_start').html(startStr);
                        $('#id_frm_crt_time_end').html(endStr);
                        $('#btn_start_trig').attr('data-time', startStr);
                        $('#btn_end_trig').attr('data-time', endStr);
